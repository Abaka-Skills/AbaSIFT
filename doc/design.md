...
- **Deterministic dump paths:** an explicitly configured `target` gives
  ...
  `dump/<mmddyyyy>/{job_id}/{node}/` (decision #34): convenient for interactive runs,
...
| 34 | `DataDumper.target` defaults to the **relative, dated** `dump/<mmddyyyy>/`; an explicit `target` is used **verbatim and undated**; free mode needs an explicit `""` | Requested: no absolute paths hardcoded in a YAML, and a tidy dated tree for interactive runs. Splitting the behaviour keeps hard-rule #10 (no timestamps ⇒ idempotent retries) exactly where it matters — every YAML an external splitter generates sets `target`. The date is read once from the job start time so all dumpers agree across midnight, and is recorded as `job.date` so the tree is reproducible. Making *free* the opt-in mode also means the destructive branch is never the default |
