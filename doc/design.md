...
The package was named `abasift` partway through implementation — see decision log #15.
...
- The abstractions: `LazyRaw`, `Sample`/`Batch`, `ArtifactUnion`, `Report`,
  ...
  and hosted views of a pipeline and of a running job (decisions #35, #36).
...
- **Deterministic dump paths:** an explicitly configured `target` gives
  `uri = f(job_id, node, key)` — no timestamps — so retries overwrite idempotently.
  When `target` is omitted the default is the dated, *relative*
  `dump/<unix ts>/{job_id}/{node}/` (decision #34): convenient for interactive runs,
  while anything generated by the external splitter sets `target` and keeps the
  idempotency guarantee. Free mode requires an explicit empty `target`.
...
| 34 | `DataDumper.target` defaults to the **relative, per-run** `dump/<unix ts>/`; an explicit `target` is used **verbatim and unstamped**; free mode needs an explicit `""` | Requested: no absolute paths hardcoded in a YAML, and a self-contained tree per interactive run. Splitting the behaviour keeps hard-rule #10 (no timestamps ⇒ idempotent retries) exactly where it matters — every YAML an external splitter generates sets `target`. The default deliberately gives that up in exchange for never colliding with an earlier run; the cost is directories that accumulate until a human clears them, which is acceptable for interactive use and not for production. The stamp is read once from the job start time so all dumpers in a DAG agree, and is recorded as `job.started_unix` so the tree is reproducible. Making *free* the opt-in mode also means the destructive branch is never the default |
| 35 | Two hosted views, never a generated file: `abasift vis <yaml>` serves what a pipeline **is** (re-reading the YAML and re-importing edited kernels per request), `abasift run <yaml> --vis` serves what a job **is doing** | Requested. A generated page is a snapshot that is wrong the moment the YAML changes, with nothing to say so — so there is no generate step and no file to go stale; the browser polls a state token and swaps its own `<main>`. Deriving the boxes by `inspect`ing the classes the YAML imports (rather than transcribing them) means a renamed parameter or a moved method changes the picture on the next poll, or the YAML visibly fails to load. Signatures are the payload: which interface a node implements (`iter_batches` for node 0, `sift` for a `SampleKernel`, `run_mutating` for the dumper) and which methods are the kernel's *own* versus inherited is the clearest statement of the framework/author division of labour. Hot reload is scoped to the kernels a YAML names and their re-exporting packages — never the core contracts, since a second `Kernel` class would break every `issubclass` check in the validator — and `RunView` reloads nothing at all, because a running job is executing the classes it already loaded |
| 36 | The executor takes an optional `observer(event, **payload)` and emits six events (`job_started`, `batch_started`, `node_started`, `node_finished`, `batch_merged`, `job_finished`); `_emit` swallows anything the observer raises | The live view needs progress the report cannot give it — the report only exists once the job ends. A callback keeps the dependency pointing the right way: the executor imports nothing from `vis` and is unaware anything is watching. Swallowing observer exceptions keeps hard rule "a job always completes and reports" intact — telemetry must never be load-bearing. `batch_merged` carries the *batch's* fragment rather than the job report, so a watcher accumulates counters instead of rescanning every sample per batch. Granularity is per node per batch: per-sample progress would need a kernel-level callback, which is not part of the `Kernel` contract |
| 37 | `SampleKernel`'s per-sample hook is named `sift`, not `check`; `Check`, `check_name` and the report's `checks` skeleton keep their names | The method does not only judge — it decodes, measures and derives per-sample artifacts, and *then* judges; `check` under-described half of what every real kernel does there. The record it returns genuinely is a judgement, and `samples.<id>.checks.<node/check>` is a consumed contract, so the rename stops at the method |
